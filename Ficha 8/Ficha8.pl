...
nulo(np9).
+servico(S,E) :: ( solucoes( S,
                             (servico(S,zulmira),nao(nulo(S))),
                             L ), 
                   comprimento(L,N), 
                   N == 0 ).
...
%--------------------------------- - - - - - - - - - -  -  -  -  -   -
% Invariante que impede o registo de atos médicos em dias feriado

+ato(A,E,U,D) :: ( solucoes( D,
                              (ato(A,E,U,D)),
                              S ),
                   nao(pertence(feriado,S)) ).

%--------------------------------- - - - - - - - - - -  -  -  -  -   -
% Invariante que impede a remoção de profissionais com atos registados

-servico(_,E) :: ( solucoes( E, 
                              ato(_,E,_,_), 
                              S ),
				   comprimento(S,N),
                   N == 0 ).
